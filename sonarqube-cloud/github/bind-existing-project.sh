...
if [[ -z $SONARQUBE_TOKEN || -z $ORGANIZATION || -z $PROJECT_KEY || -z $REPOSITORY_URL || -z $DEVOPS_TOKEN ]]; then
...
if ! project_exists "$PROJECT_KEY" "$SONARQUBE_TOKEN" "$SONARQUBE_URL"; then
    log_message "ERROR" "Project $PROJECT_KEY does not exist in SonarQube Cloud"
    exit 1
fi

log_message "INFO" "Binding project $PROJECT_KEY to GitHub repository: $REPOSITORY_OWNER/$REPOSITORY_NAME"
